## 🧪 React Testing Library + Jest Learning Roadmap

### 🏁 **Phase 1 – Foundations**

**Goal:** Understand how to write and run basic component tests.
**Topics:**

1. ✅ _Setting up Jest and React Testing Library_
2. ✅ _Testing Components (Rendering, Queries, Assertions, Interactions)_ ← **Start here**
3. 🧱 _Testing props, conditional rendering, and event handlers_

---

### 🌐 **Phase 2 – Mocking and API Testing**

**Goal:** Learn how to test components that depend on external data.
**Topics:**

4. Mocking API calls (`fetch`, `axios`) using `jest.mock()`
5. Testing async behavior (`findBy`, `waitFor`, `mockResolvedValue`)

---

### 📝 **Phase 3 – Forms and Inputs**

**Goal:** Learn to simulate user input and validate behavior.
**Topics:**

6. Testing forms (typing, submitting, validation, reset, etc.)
7. Mocking form submission handlers

---

### ⚙️ **Phase 4 – State and Context**

**Goal:** Test components that use global state or context.
**Topics:**

8. Testing Context API
9. Testing Redux or Zustand stores
10. Using custom render helpers (wrapping with providers)

---

### 🔐 **Phase 5 – Authentication**

**Goal:** Learn how to test protected components or login flows.
**Topics:**

11. Mocking authentication hooks (e.g., Firebase, Auth0, custom hooks)
12. Testing redirects for logged-in vs logged-out users

---

### 🧭 **Phase 6 – Routing**

**Goal:** Verify navigation and route-based rendering.
**Topics:**

13. Testing `react-router-dom` components (Link, Navigate, useNavigate)
14. Mocking navigation + testing route transitions

---

### 🧰 **Phase 7 – Best Practices**

**Goal:** Build confidence in test structure and coverage.
**Topics:**

15. Writing clean and maintainable test code
16. Testing accessibility and performance hints
17. Debugging failed tests efficiently
